...
**Session drawer amendment.** Create session uses the same right-side, full-height 1060px drawer as View session, with fixed header actions and one scrolling body. Only **Driver and Programme** are chosen; **Coach** displays the current manager read-only. Omit per-session Due, Training lesson and Reason inputs during creation. **Session due period** is a saved global setting in Programmes → Automation, defaulting to seven days for new sessions; unsaved drafts and later changes do not alter existing deadlines. Select matching driver/programme videos automatically, including every camera clip. Evidence sits below Coach in a compact table; previews start closed. Expanding a row shows its video and location side by side, stacked on narrow screens. Add events expands an inline searchable picker in the same drawer; Add selected stages its choices, while Cancel or Escape discards them without closing creation. Commit evidence only on Create. Creation preserves the source page and opens the resulting session in place. Session review uses compact **Events / Videos** counts, no Messages count, a behaviour-named collapsed breakdown and a taller Conversation area. The [reference review](docs/session-drawer-review.md) records the Mobbin screens inspected for this update.
...
This document defines one shared system for Automation Centre, Sessions, Session workspace, Programmes, Drivers, Groups and Learning, including programme Configuration and fleet-wide Automation. It incorporates the requested move away from blue to an orange direction inspired by Cloudflare. Elevate values reflect the supplied baseline and the explicit user amendments below; they are not sampled competitor tokens. The deliverable is a single Markdown document; reference screenshots are embedded from their public source URLs and require network access to display.

## User amendment — version 1.1, 7 September 2026

This repository specification incorporates the user's follow-up direction: brighten neutral surfaces, reduce all text and line-height tokens by 15%, use one petrol colour for attention-category dots and bars, move KPI explanations into accessible info hints, put long chart source details inside **Summary and data**, and organize Activity around the joined 70/30 weekly-coaching/attention panels with zero gutter and one divider, the All-programme comparison/Event rates module and collapsed outcomes. These decisions supersede the corresponding version 1.0 values and presentation rules throughout this document. The original supplied file in Downloads is historical and remains unchanged.
...
## Product amendment — navigation merge, 7 September 2026

Primary navigation has six desktop destinations: **Automation Centre, Sessions, Programmes, Drivers, Learning and Driver app**. At **680px and below**, the bottom bar keeps Automation Centre, Sessions, Programmes and Drivers, with **More** exposing Learning and Driver app. Driver app is a required clickable destination, not a route-only page; preserve the user’s KEEP markers in `dist/index.html` and the instructions in `AGENTS.md`. Programmes combines the former Analytics and Programs workspaces into **Activity, Learning, Configuration and Automation**. Drivers contains **Drivers / Groups**. The full Learning library remains a destination; programme Learning is its mapped subset. Direct `#driver-app` and standalone `/driver/` access remain supported. This amendment replaces prior navigation expectations; historical research retains its original terminology.

Programmes opens **All programmes / Activity** with seven current summary values: Identified, In progress, Needs review, Completed, Automated sessions, One-on-one sessions and Elevate score. Selecting a programme names the seventh tile **`<programme name> score`**, matching the weekly-chart label; All programmes keeps **Elevate score**. The first four reconcile the current source ledger. The two method totals include active, review and completed sessions; in-progress method counts are labelled explicitly wherever shown. Needs review groups Overdue, Repeated and Replied actual sessions and focuses the attention panel joined to the Activity chart. Other count shortcuts preserve programme/period scope when opening Sessions. Do not reintroduce pending Session needed records or a second session ledger on Programmes.

Activity follows **joined weekly coaching/attention row → All-programme comparison, when applicable → collapsed outcomes**. Activity joins **weekly coaching at 70% and drivers needing attention at 30%** into one desktop row with **zero gutter and a single divider**. The full attention list scrolls independently; its count sits inline to the right of the heading. The joined panels share their top and bottom edges and stack on narrow screens. The selected programme’s KPI and weekly-chart score label is **`<programme name> score`** (for example, “Speeding score”); All programmes keeps **“Elevate score”**. The separate selected-programme event-rate Trend/Comparison card is removed. All programmes retains its Coaching/Event rates comparison and the underlying recorded rates, source endpoints and equivalent data. All programmes retains one native Coaching/Event rates switch, source counts, every programme, signed rate changes and recorded scores. Selected programmes show weekly coaching and recorded outcomes without a separate event-rate Trend/Comparison card. Do not duplicate the full comparison as simultaneously visible chart and table or average programme rates into a fleet rate without exposure. Completion fraction/percentage and its meaningful meter stay in Completed KPI help, with no percentage or meter when the denominator is absent.

Current counts come from the selected-period ledger; chart history has separate provenance. User-authorized programme weekly examples must visibly say **Sample history**, give their dates and repeat that provenance in the accessible description and equivalent data. Their current endpoint uses This week ledger counts, without accumulating a longer reporting window; their earlier shape is not claimed as observed ledger history. This week retains eight weeks of explicitly dated context, and Last 4/8 weeks shows four/eight snapshots. Unknown/new programme history has no borrowed fallback. Fleet weekly snapshots and the overall **Elevate score** remain independent of selected programme sample history. Recorded prototype Program scores use 0–100, higher is safer, and are not yet computed from rule weights; missing values stay unavailable. The weighted overall programme roll-up and verified LMS/cohort impact remain unimplemented.

Recorded outcomes is initially collapsed and preserves the available eligible/improved/repeated source facts. Its **Undated sample** cue and expanded unavailable observation window identify independence from the reporting period. Keep long methodology inside the disclosure and each chart's Summary and data with that chart. Before/After observations are not causal coaching evidence. The shared chart expands width, gutters and geometry at 200% text zoom; keep equivalent data and labelled local scrolling.

Activity and Groups use Programme/Period controls and one exact date caption. Groups applies those controls to coaching workload; recorded group size and **Overall Elevate score** keep explicit overall scope. Learning and Configuration use only Programme scope, with no irrelevant reporting strip or time controls. Automation is fleet-wide and reads **Applies to every programme**. Configuration keeps its existing locally persisted programme/rule/threshold/escalation/coach edits. Automation retains draft → preview → save/activate and discard behavior. This merge changes neither execution model.

Programme details remain a full page. Driver, Group and Session drawers retain the uniform 1060px contract and source return behavior; do not restore a programme drawer or Quick view. Group/Activity → programme → session returns preserve scope, scroll and opener. Canonical `#programs` opens Activity; tab keys are `content`, `configuration`, `automation`; `#drivers` opens the independent directory and `driversTab=groups` selects Groups. Legacy `programTab=groups` redirects to Drivers / Groups. Legacy Analytics routes migrate to Activity, Groups or Drivers as appropriate. Preserve period, selected programme, directory filters and linked record context while migrating URLs. See [the local contract](DESIGN_SYSTEM.md) and [current review scope](docs/programs-review.md). **Historical validation:** the earlier September 7 merge passed syntax/static, workspace/source reconciliation, programme/record/configuration/Driver app, browser, design and cleanup suites. That run predates the current six-destination, zero-gutter and selected-chart correction; do not treat it as validation of these latest changes. `test:workspace` covers all ten programmes at 1/4/8 weeks and sample-history provenance; shared design checked 126 tables and 2,860 cells plus mobile and 200% text zoom. These are local prototype checks, not live-engine, causal-impact or whole-product accessibility certification. The linked current acceptance record lists actual commands and scope; historical results remain separate.

## UI cleanup amendment — 7 September 2026

Use views on the left and one control group on the right: applicable programme/period, search, then Filters. Programmes Activity and Groups have reporting controls; Learning and Configuration retain only Programme scope; Automation shows fleet-wide scope. Activity owns the seven-value summary strip. These contextual exceptions do not change other pages. Nested table filters stay beside their dataset. On narrow screens the group wraps below the views in the same order. Page titles identify the workspace; selectors identify its scope. Keep one exact page-period caption where applicable and source-specific chart dates, without repeating status badges, review composition, result counts or generic navigation links elsewhere. Critical scope labels, native accessible names, concise status words and recorded data remain available.
...
Use native radios in a labelled fieldset for segmented values. Use real links with `aria-current="page"` in navigation. Workspace view tabs use `role="tablist"`, `role="tab"`, `aria-selected`, `aria-controls` and a labelled panel. Use arrow keys/Home/End for focus, Enter/Space to activate, and one tab stop in the tablist. Preserve the view in the URL. [WAI tab pattern](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/).
...
| Session workspace | Compact Events / Videos counts → neutral conversation panel → same evidence drawer header. Inbound/outbound bubbles use surface/surface-soft and author labels, never brand tint. |
| Programmes: Activity | Seven current values → tabs left, Programme/Period/date right → joined 70/30 weekly coaching/attention row, zero gutter and one divider, count beside attention heading → All-programme Coaching/Event rates comparison when applicable → collapsed outcomes. Selected scope names its score for the programme and has no separate event-rate chart. |
| Drivers: Groups | Tabs left, Programme/Period/date right → scoped coaching workload and shared group comparison → group drawer. Recorded group score/size remain clearly overall. |
| Drivers: Drivers | Independent directory with existing summary, search/filter controls and shared table → driver portfolio → session; preserve directory scope and return context. |
| Programmes: Learning / Configuration | Page header → tabs left and Programme selector right → mapped lessons or existing local configuration. No Activity KPIs or time controls; retain period state for reporting tabs. |
| Programmes: Automation | Page header → tabs and fleet-wide scope → existing mode/cadence/completion draft, preview, discard and activation flow. No programme/period filter. |
| Learning library | Meaningful library measures → shared filter/search → neutral content cards using the same card shell. |
...
| 9 | Give programme reports denser rows and configuration larger, differently tinted tables. | One 48px nominal row, shared padding/type, neutral header and common states. |
...
5. **Status dictionary and shared table.** One State column per session row, Coach in place of Method, Started/Completed/Due dates, fix semantic pills, then migrate Sessions, Programmes, Drivers and configuration tables together.
