...
function updateWorkspaceConversation(session) {
  ...
  const course = typeof ProgramSetup !== 'undefined' && session.lesson !== null ? ProgramSetup.courseForProgram(session.categoryId) : null;
  const lesson = course ? course.title : session.lesson || lessons.find(item => item.category === session.category)?.title;
  const courseMeta = course ? [course.videoSeconds ? (course.videoSeconds % 60 === 0 ? course.videoSeconds / 60 + ' min video' : course.videoSeconds + ' sec video') : '', course.questions?.length ? course.questions.length + '-question quiz' : ''].filter(Boolean).join(' · ') : '';
  if (progress) progress.innerHTML = automated ? '<section class="sw-training-progress" aria-labelledby="sw-training-title"><h3 id="sw-training-title">' + escapeHtml(lesson || 'Automated lesson') + '</h3>' + (course ? '<p class="caption">' + escapeHtml(courseMeta) + ' · <button class="text-link" type="button" data-tl-open="' + escapeHtml(course.id) + '" aria-haspopup="dialog">Preview course</button></p>' : '') + '<ol class="sw-training-steps"><li>' + uiIcon(session.lessonWatched ? 'check' : 'play') + '<span>' + (session.lessonWatched ? 'Video watched' : 'Waiting for video review') + '</span></li>' + (course && course.questions?.length ? '<li>' + uiIcon(session.quizPassed ? 'check' : 'clock') + '<span>' + (session.quizPassed ? 'Quiz passed · ' + course.questions.length + ' of ' + course.questions.length : 'Quiz not yet passed') + '</span></li>' : '') + '<li>' + uiIcon(session.lessonAcknowledged ? 'check' : 'clock') + '<span>' + (session.lessonAcknowledged ? 'Acknowledged' : 'Awaiting acknowledgement') + '</span></li></ol><p class="caption">The driver watches' + (course && course.questions?.length ? ', passes the quiz' : '') + ' and acknowledges. Start one-on-one if a conversation is needed.</p></section>' : '';
