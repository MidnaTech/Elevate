...
(function () {
  ...
  function featured() {
    return '<section class="tl-featured" id="tl-featured" aria-labelledby="tl-pack-title"><div class="tl-section-heading"><div><p class="eyebrow">Featured course series · Heavy-duty drivers</p><h2 class="section-title" id="tl-pack-title">Heavy-truck speeding</h2><p>Build safer speed decisions, from an early habit reset to judgment under pressure.</p></div>' + uiStatus(pack()?.levels?.every(level => level.media?.videoUrl) ? 'Videos ready' : 'Video production pending') + '</div><div class="tl-levels">' + (pack()?.levels || []).map(level => {
      const course = courses().find(item => item.id === level.id);
      if (!course) return '';
      return '<article class="card tl-level"><span class="caption">Level ' + level.level + '</span><h3 class="section-title">' + h(course.title) + '</h3><p>' + h(level.learningGoal) + '</p><p class="caption">' + h(level.estimatedDuration) + ' estimated · ' + h(runtime(course)) + (course.videoUrl ? ' captioned video' : ' video target') + ' · 3-question quiz</p><div class="toolbar">' + previewButton(course) + '</div></article>';
    }).join('') + '</div><p class="caption">' + (pack()?.levels?.every(level => level.media?.videoUrl) ? 'Lessons, question banks and captioned videos are ready. Estimated course times include reading and reflection.' : 'Lessons, scripts and question banks are prepared. Videos have not been produced. Estimated course times include reading and reflection.') + '</p></section>';
  }
  ...
  function videoPanel(course, material) {
    ...
    const local = course.videoUrl && !/^https?:\/\//.test(course.videoUrl);
    const player = course.videoUrl && /^(https?:\/\/|\.{0,2}\/|media\/)/.test(course.videoUrl)
      ? '<video id="tc-video" class="tc-video" controls playsinline preload="' + (local ? 'metadata' : 'none') + '"' + (course.posterUrl ? ' poster="' + h(course.posterUrl) + '"' : '') + ' aria-label="' + h(course.title + ' training video') + '" src="' + h(course.videoUrl) + '">' + (course.captionsUrl ? '<track kind="captions" srclang="en" label="English" src="' + h(course.captionsUrl) + '">' : '') + '</video><p class="caption" id="tc-video-error" role="status">' + (local ? h(runtime(course)) + ' course video' + (course.captionsUrl ? ' with captions' : '') + '. Playback begins when you choose Play.' : 'Video supplied by a linked source. Playback begins when you choose Play.') + '</p>'
