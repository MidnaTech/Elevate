...
Current workspace: Automation Centre, Sessions, Programmes, Drivers, Learning and Driver app. Drivers contains Drivers / Groups; Programmes contains Activity / Learning / Configuration / Automation. Activity joins weekly coaching and attention in a 70/30 row with zero gutter and one divider; the attention count stays beside its heading. At 680px and below, More exposes Learning and Driver app. See the [full request audit](docs/request-audit-2026-09-07.md) for completion status, local-only features and unresolved scoring rules. Run `npm run test:groups` for the moved tab and drawer paths.
...
- Learning library of coaching lessons
- Per-program configuration (rules with source, severity, weight and threshold; coaching threshold; escalation; coach routing) and fleet-wide draft-first automation mode and cadence on Programmes › Automation with previews and audit history
...
Run `npm run test:config` for [program configuration](docs/program-configuration.md): creating and deleting programs; per-rule source, severity, weight, threshold, on/off and Direct one-on-one; the program coaching threshold; Overdue, Repeated and minimum-exposure escalation settings; one-on-one coach routing (one coach or per group); the generated "What happens" text; local persistence; the Programmes › Automation tab and legacy `#settings` route; the single-state session vocabulary (no "Awaiting driver", "Awaiting session" or "Session needed"); and the Automation Centre without a featured driver.
