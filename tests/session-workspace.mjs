...
try {
  ...
  // Creation keeps details and evidence in one column. Its inline picker stages additions;
  // only Create links evidence or assigns an unassigned event to the driver.
  await page.goto(base + '/#sessions');
  const manualDialog = page.locator('#training-dialog');
  const manualPicker = manualDialog.locator('#manual-event-picker');
  const manualIncluded = () => manualDialog.locator('[data-manual-event]:checked');
  const openManualPicker = async (scope = 'driver') => {
    await manualDialog.locator('[data-manual-browse-events]').click();
    assert.equal(await manualPicker.isVisible(), true, 'Add events expands inside the creation drawer');
    assert.equal(await page.evaluate(() => Boolean(document.getElementById('session-event-browser')?.open)), false, 'Creation never opens the session-view modal event browser');
    await manualPicker.locator('[data-manual-picker-scope="' + scope + '"]').click();
  };
  const applyManualPicker = async () => {
    await manualPicker.locator('[data-manual-picker-apply]').click();
    assert.equal(await manualPicker.isVisible(), false);
    assert.equal(await manualDialog.locator('[data-manual-browse-events]').evaluate(node => node === document.activeElement), true, 'Adding evidence returns focus to Add events');
  };
  const assertManualStack = async width => {
    const driverField = await manualDialog.locator('#manual-driver-select').boundingBox();
    const programmeField = await manualDialog.locator('#manual-category-select').boundingBox();
    const coach = await manualDialog.locator('.manual-coach-summary').boundingBox();
    const evidence = await manualDialog.locator('.manual-evidence-pane').boundingBox();
    assert.ok(coach.y >= Math.max(driverField.y + driverField.height, programmeField.y + programmeField.height) - 1, 'Coach follows Driver and Programme at ' + width + 'px');
    assert.ok(evidence.y >= coach.y + coach.height - 1, 'Evidence follows Coach at ' + width + 'px');
    assert.ok(Math.abs(driverField.width - programmeField.width) < 2 && evidence.width >= driverField.width - 2, 'Evidence uses the same full content column at ' + width + 'px');
  };
  const assertManualMedia = async (container, width) => {
    const video = await container.locator('.sw-video').boundingBox();
    const map = await container.locator('.sw-map').boundingBox();
    assert.ok(video && map, 'Expanded evidence retains both video and map');
    if (width >= 1024) {
      assert.ok(video.x + video.width <= map.x + 2 && Math.abs(video.y - map.y) < 2, 'Video and map share one row on desktop');
    }
  };
  ...
  const preselected = await manualIncluded().count();
  assert.equal(preselected, 2, 'The two dated events in this driver/programme period are selected automatically');
  assert.equal(await manualDialog.locator('.manual-evidence-expanded:visible').count(), 0, 'All event previews start collapsed');
  assert.equal(await manualDialog.locator('[data-manual-preview][aria-expanded="false"]').count(), preselected);
  const evidenceTable = manualDialog.getByRole('table', { name: 'Session evidence', exact: true });
  assert.deepEqual((await evidenceTable.locator('thead th').allTextContents()).map(text => text.trim()), ['', 'Event', 'Recorded', 'Videos'], 'Evidence uses one native table with include, event, recorded and video columns');
  const evidenceRows = await evidenceTable.locator('tbody tr:has([data-manual-event])').evaluateAll(rows => rows.map(row => ({ id: row.querySelector('[data-manual-event]').dataset.manualEvent, videos: Number(row.cells[3].textContent) })));
  assert.deepEqual(evidenceRows, await page.evaluate(ids => ids.map(id => ({ id, videos: eventClips(evidenceEvent(id)).length })), evidenceRows.map(row => row.id)), 'Video counts come from every camera on each unique event');
  const firstPreview = manualDialog.locator('[data-manual-preview]').first();
  await firstPreview.click();
  assert.equal(await firstPreview.getAttribute('aria-expanded'), 'true');
  assert.equal(await firstPreview.evaluate(node => node.closest('tr').nextElementSibling.classList.contains('manual-evidence-expanded')), true, 'Preview expands immediately under its event row');
  await assertManualMedia(manualDialog.locator('.manual-evidence-expanded:visible'), 1440);
  assert.equal(await manualIncluded().count(), preselected, 'Previewing does not change event inclusion');
  const expandedId = await firstPreview.getAttribute('data-manual-preview');
  await evidenceTable.locator('[data-table-sort="1"]').click();
  assert.equal(await evidenceTable.locator('[data-manual-preview="' + expandedId + '"]').evaluate(node => {
    const row = node.closest('tr');
    return row.nextElementSibling.dataset.detailFor === row.id && !row.nextElementSibling.hidden;
  }), true, 'Sorting keeps the expanded media immediately after its parent event');
  assert.equal(await evidenceTable.locator('tbody tr:has([data-manual-event])').evaluateAll(rows => rows.every(row => row.nextElementSibling.dataset.detailFor === row.id)), true, 'Sorting also retains every collapsed detail row with its parent');
  await manualDialog.locator('[data-manual-preview="' + expandedId + '"]').click();
  assert.equal(await manualDialog.locator('.manual-evidence-expanded:visible').count(), 0, 'The same row control collapses its preview');
  ...
  assert.equal(await manualPicker.locator('.manual-evidence-expanded:visible').count(), 0, 'The inline picker also starts with closed previews');
  const manualDriverEventIds = await manualPicker.locator('[data-manual-picker-select]').evaluateAll(nodes => nodes.map(node => node.dataset.manualPickerSelect));
  assert.ok(manualDriverEventIds.length >= 2);
  assert.equal(await page.evaluate(ids => ids.every(id => evidenceEvent(id).person === 'Rowan Hall' && evidenceEvent(id).categoryId === 'distraction'), manualDriverEventIds), true, 'This driver shows only the chosen driver and programme');
  assert.equal(await manualPicker.locator('[data-manual-picker-select]:disabled:checked').count(), 2, 'Already included events cannot be added twice');
  await manualPicker.locator('[data-manual-picker-scope="unassigned"]').click();
  const manualUnassignedIds = await manualPicker.locator('[data-manual-picker-select]').evaluateAll(nodes => nodes.map(node => node.dataset.manualPickerSelect));
  assert.ok(manualUnassignedIds.includes('event:unassigned-1') && manualUnassignedIds.includes('event:unassigned-6'));
  assert.equal(await page.evaluate(ids => ids.every(id => !evidenceEvent(id).person && evidenceEvent(id).categoryId === 'distraction'), manualUnassignedIds), true, 'Unassigned is still limited to the selected programme');
  await manualPicker.locator('[data-manual-picker-preview="event:unassigned-1"]').click();
  assert.equal(await manualPicker.locator('[data-sw-camera]').count(), 2, 'Both source cameras remain one event in the inline picker');
  await assertManualMedia(manualPicker.locator('.manual-evidence-expanded:visible'), 1440);
  const cameraIds = await manualPicker.locator('[data-sw-camera]').evaluateAll(nodes => nodes.map(node => node.dataset.swCamera));
  await manualPicker.locator('[data-sw-camera="' + cameraIds[1] + '"]').locator('..').click();
  assert.equal(await manualPicker.locator('[data-sw-camera="' + cameraIds[1] + '"]').getAttribute('aria-pressed'), 'true', 'The expanded event can switch cameras without losing its row');
  await manualPicker.locator('[data-manual-picker-select="event:unassigned-1"]').check();
  await manualPicker.locator('[data-manual-picker-select="event:unassigned-6"]').check();
  assert.equal(await manualIncluded().count(), 2, 'Picker checkboxes are separate from the session draft until Add selected');
  assert.equal(await manualDialog.locator('[data-confirm-manual-session]').isDisabled(), true, 'Creation cannot bypass an open picker with unapplied choices');
  await manualPicker.locator('#manual-event-search').fill('No such incident anywhere');
  assert.equal(await manualPicker.locator('[data-manual-picker-select]').count(), 0);
  assert.match(await manualPicker.locator('#manual-event-results').textContent(), /No matching events/);
  await manualPicker.locator('#manual-event-search').fill('');
  assert.equal(await manualPicker.locator('[data-manual-picker-select]:checked').count(), 2, 'Filtering does not discard staged selections');
  await manualPicker.locator('[data-manual-picker-scope="driver"]').click();
  await manualPicker.locator('[data-manual-picker-scope="unassigned"]').click();
  assert.equal(await manualPicker.locator('[data-manual-picker-select]:checked').count(), 2, 'Scope switches preserve staged selections');
  await manualPicker.locator('[data-manual-picker-cancel]').click();
  assert.equal(await manualPicker.isVisible(), false);
  assert.equal(await manualIncluded().count(), 2, 'Cancel discards picker changes without changing the draft');
  assert.equal(await manualDialog.locator('[data-manual-browse-events]').evaluate(node => node === document.activeElement), true);
  await openManualPicker('unassigned');
  assert.equal(await manualPicker.locator('[data-manual-picker-select]:checked').count(), 0, 'A cancelled selection is not revived');
  await manualPicker.locator('[data-manual-picker-select="event:unassigned-1"]').check();
  await page.keyboard.press('Escape');
  assert.equal(await manualPicker.isVisible(), false);
  assert.equal(await manualDialog.evaluate(node => node.open), true, 'Escape closes the inline picker before the creation drawer');
  assert.equal(await manualIncluded().count(), 2);
  assert.equal(await page.inputValue('#manual-driver-select'), 'Rowan Hall');
  assert.equal(await page.inputValue('#manual-category-select'), 'distraction');
  await openManualPicker('unassigned');
  await manualPicker.locator('[data-manual-picker-select="event:unassigned-1"]').check();
  await page.selectOption('#manual-driver-select', 'Priya Singh');
  assert.equal(await manualPicker.isVisible(), false, 'Changing the driver closes and discards the picker');
  assert.equal(await page.evaluate(() => [...manualCoachingState.selectedEvents].every(id => evidenceEvent(id).person === 'Priya Singh')), true, 'A driver change cannot retain another driver’s evidence');
  await page.selectOption('#manual-driver-select', 'Rowan Hall');
  await openManualPicker('unassigned');
  await manualPicker.locator('[data-manual-picker-select="event:unassigned-1"]').check();
  await page.selectOption('#manual-category-select', 'following');
  assert.equal(await manualPicker.isVisible(), false, 'Changing the programme also discards staged picker choices');
  assert.equal(await page.evaluate(() => [...manualCoachingState.selectedEvents].every(id => evidenceEvent(id).categoryId === 'following')), true);
  await page.selectOption('#manual-category-select', 'distraction');
  await openManualPicker('unassigned');
  await manualPicker.locator('[data-manual-picker-select="event:unassigned-1"]').check();
  await manualPicker.locator('[data-manual-picker-select="event:unassigned-6"]').check();
  await applyManualPicker();
  assert.equal(await manualIncluded().count(), 4, 'Add selected adds multiple unique events to the draft');
  await openManualPicker('unassigned');
  assert.equal(await manualPicker.locator('[data-manual-picker-select]:disabled:checked').count(), 2, 'Reopening marks both additions as included instead of duplicating them');
  assert.equal(await manualPicker.locator('[data-manual-picker-apply]').isDisabled(), true);
  await manualPicker.locator('[data-manual-picker-cancel]').click();
  ...
  assert.equal(await page.evaluate(() => evidenceEvent('event:unassigned-6').person), null);
  ...
  assert.equal(created.events.length, 4);
  assert.equal(new Set(created.events).size, created.events.length, 'The created session has no duplicate event records');
  assert.deepEqual([...created.clips].sort(), await page.evaluate(ids => ids.flatMap(id => eventClips(evidenceEvent(id)).map(clip => clip.id)).sort(), created.events), 'Create attaches exactly all cameras from every selected event');
  ...
  for (const width of [1440, 1024, 768, 390, 320]) {
    ...
    await assertManualStack(width);
    assert.equal(await manualDialog.locator('.manual-evidence-expanded:visible').count(), 0, 'Prefilled creation still starts with closed previews at ' + width + 'px');
    await manualDialog.locator('[data-manual-preview]').first().click();
    await assertManualMedia(manualDialog.locator('.manual-evidence-expanded:visible'), width);
    assert.ok(await manualDialog.evaluate(node => node.scrollWidth <= node.clientWidth + 1), 'Creation stays within its drawer at ' + width + 'px');
    await openManualPicker('unassigned');
    assert.equal(await manualPicker.locator('.manual-evidence-expanded:visible').count(), 0);
    await manualPicker.locator('[data-manual-picker-preview="event:unassigned-1"]').click();
    assert.ok(await manualPicker.evaluate(node => node.scrollWidth <= node.clientWidth + 1), 'Inline events fit their content region at ' + width + 'px');
    assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1), 'Expanded evidence does not cause page overflow at ' + width + 'px');
    await page.keyboard.press('Escape');
    assert.equal(await manualDialog.evaluate(node => node.open), true, 'Escape dismisses only inline browsing at ' + width + 'px');
    assert.equal(await manualPicker.isVisible(), false);
    await manualDialog.locator('.manual-coaching-body').evaluate(node => { node.scrollTop = node.scrollHeight; });
  ...
  console.log('Passed: stacked manual creation with expandable evidence tables and inline staged selection, source-preserving event selection, automated private notes and explicit one-on-one handoff, linked-event defaults, grouped footage staging/cancel/share, persistent drafts/caret, lifecycle actions, missing-media honesty, and shared responsive drawer widths.');
} catch (error) {
